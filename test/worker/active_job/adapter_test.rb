      ...
      if defined?(YantraActiveRecordTestCase) && AR_LOADED
        ...
        class AdapterTest < YantraActiveRecordTestCase
          ...
          def test_cancel_jobs_bulk_cancels_only_cancellable_states
            ...
            step_running = StepRecord.create!(id: SecureRandom.uuid, workflow_record: @workflow, klass: "JobR", state: "running") # This one won't be cancelled now
            step_succeeded = StepRecord.create!(id: SecureRandom.uuid, workflow_record: @workflow, klass: "JobS", state: "succeeded", finished_at: Time.current)
            step_failed = StepRecord.create!(id: SecureRandom.uuid, workflow_record: @workflow, klass: "StepF", state: "failed", finished_at: Time.current)
            step_cancelled_already = StepRecord.create!(id: SecureRandom.uuid, workflow_record: @workflow, klass: "StepC", state: "cancelled", finished_at: Time.current)

            step_ids_to_cancel = [
              step_pending.id,
              step_enqueued.id,
              step_running.id, # Included in list, but adapter logic should skip it
              step_succeeded.id,
              step_failed.id,
              step_cancelled_already.id
            ]
            ...
            assert_equal "running", step_running.reload.state # <<< Running job should NOT be cancelled
            ...
            assert_nil step_running.finished_at, "Running job finished_at should remain nil" # <<< Added check
