...
module Yantra
  module Core
    ...
    class WorkflowRetryService
      ...
      # @param repository [#get_workflow_jobs, #update_job_attributes, #find_job] The persistence adapter.
      ...
      def call
        ...
        puts "INFO: [WorkflowRetryService] Found #{failed_jobs.size} failed jobs to retry: #{failed_jobs.map(&:id)}."
        reenqueue_jobs(failed_jobs)
      end
      ...
      def find_failed_jobs
        repository.get_workflow_jobs(workflow_id, status: :failed)
      ...
      def reenqueue_job(job)
        ...
        job_update_success = repository.update_job_attributes(
          job.id,
          {
        ...
        if job_update_success
          ...
          latest_job_state = repository.find_job(job.id)&.state || 'unknown'
          puts "WARN: [WorkflowRetryService] Failed to update job #{job.id} state to enqueued (maybe state changed concurrently? Current state: #{latest_job_state}). Skipping enqueue."
          false
        end
