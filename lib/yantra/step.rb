  ...
  class Step
    ...
    def parent_outputs
      ...
      if @parent_ids && !@parent_ids.empty?
        ...
        unless @id && @repository && @repository.respond_to?(:get_dependencies_ids)
          Yantra.logger&.warn { "[Step#parent_outputs] Cannot fetch parent IDs dynamically for step #{@id.inspect}: missing step ID or repository does not support get_dependencies_ids."} if Yantra.logger
          return @_parent_outputs_cache = {} # Cannot proceed
        end

        # Perform dynamic lookup
        begin
          ids_to_fetch = @repository.get_dependencies_ids(@id)
          ...
          Yantra.logger&.error { "[Step#parent_outputs] Error fetching parent IDs using get_dependencies_ids for step #{@id.inspect}: #{e.message}" } if Yantra.logger
